...
class User extends UserCommon
{
    ...
    {
        return [
            [['use_nombre', 'use_apellido', 'uge_id', 'use_telefono','rol_id', 'pai_id', 'email'], 'required'],
            [['pai_id', 'status', 'created_at', 'uge_id','rol_id', 'updated_at'], 'integer'],
    ...
    public function signup() {
        ...
        if(!$this->save()){
            ...
            $userEmail = User::find()->where(['id'=>$this->id])->one();

            if($bandera == true){
                $this->sendEmail($userEmail);
            }
            if($this->rol_id == 2 && $this->use_estado == 2){
                $this->sendEmailInvestigador($userEmail);
            }
            if($this->rol_id == 2 && $this->use_estado == 3){
                $this->sendEmailInvestigadorNegado($userEmail);
            }
            if($this->rol_id == 1){
                $this->sendEmailAdministrador($userEmail);
            }
            return true;
        }
    ...
    protected function sendEmailInvestigador($user)
    {
        return Yii::$app
            ->mailer
            ->compose(
                ['html' => 'emailAprobadoInvestigador-html', 'text' => 'emailAprobadoInvestigador-text'],
                ['user' => $user]
            )
            ->setFrom([Yii::$app->params['supportEmail'] => Yii::$app->name . ' - DYAC'])
            ->setTo($this->email)
            ->setSubject('Solicitud de aprobación - Investigador')
            ->send();
    }

    protected function sendEmailInvestigadorNegado($user)
    {
        return Yii::$app
            ->mailer
            ->compose(
                ['html' => 'emailNegadoInvestigador-html', 'text' => 'emailNegadoInvestigador-text'],
                ['user' => $user]
            )
            ->setFrom([Yii::$app->params['supportEmail'] => Yii::$app->name . ' - DYAC'])
            ->setTo($this->email)
    ...
    protected function sendEmailAdministrador($user)
    {
        return Yii::$app
            ->mailer
            ->compose(
                ['html' => 'emailAdministrador-html', 'text' => 'emailAdministrador-text'],
                ['user' => $user]
            )
            ->setFrom([Yii::$app->params['supportEmail'] => Yii::$app->name . ' - DYAC'])
            ->setTo($this->email)
            ->setSubject('Administrador de la aplicación')
            ->send();
    }
